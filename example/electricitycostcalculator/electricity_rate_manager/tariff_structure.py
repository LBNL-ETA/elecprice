...
class TariffType(Enum):
    ...
    NEM_ENERGY_CHARGE = 'NEM_NBC'
    NEM_ENERGY_CREDIT = 'NEM_NSC'
...
class TariffBase(object):
    ...
    def compute_bill(self, df, data_col=None):
        ...
        while t_s <= t_e:
            ...
            if month >= 13:
                month = 1
                year += 1
                yearly_bill = self.compute_yearly_bill(df, data_col)
                ret[t_s.strftime("%Y")]=yearly_bill #how to get the demand charges
    ...
    @abstractmethod
    def compute_yearly_bill(self, df, data_col=None):
        """
        Compute the yearly bill due to the power/energy consumption in df
        :param df: a pandas dataframe
        :param data_col: the column label containing the data
        :return: a tuple (float, float) -> (value, cost), representing the bill and the corresponding metric linked to the cost
        """
...
class FixedTariff(TariffBase):
    ...
    def compute_monthly_bill(self, df, data_col=None):
        ...
        elif self.__rate_period == TariffElemPeriod.YEARLY:
            print('yearly fixed rate')
    ...
    def compute_yearly_bill(self, df, data_col=None):
        ...
        d = {0.0: {'mask': 
                   [True, True, True, True, True, True, True, True, True, True, True, True, 
                    True, True, True, True, True, True, True, True, True, True, True, True], 
                   'max-demand': 0, 'max-demand-date': pd.datetime(2020, 5, 14, 9, 0)}}
...
class TouEnergyChargeTariff(TimeOfUseTariff):
    ...
    def compute_monthly_bill(self, df, data_col=None):
        ...
        # TODO:  change everything to arrays instead of pandas dataframes
        ...
        for idx, df_day in df.groupby(df.index.date):
            ...
            for i in range(24): 
                ...
                hrly_energy = df_values_in_hour / mult_energy_unit
            ...
            # Cumulate the bill over the month
                ...
                #print(daily_rate)
                if isinstance(daily_rate[0],float) | isinstance(daily_rate[0],int): #only tou
...
class NEMChargeTariff(TimeOfUseTariff):
    """
    This class represents a Non-bypassable charges in the NEM (net energy metering) tariff
    """
    ## TOU Rate Schedule
    ...
    def __init__(self, dates, time_schedule, unit_metric=TariffElemMetricUnit.EN_KWH, unit_cost=TariffElemCostUnit.DOLLAR, name=None):
        ...
        super(NEMChargeTariff, self).__init__(dates, time_schedule, unit_metric, unit_cost, name)
...
class NEMCreditTariff(TimeOfUseTariff):
    """
    This class represents a net surplus credits in the NEM (net energy metering) tariff OPTION 1 
    """
    ## TOU Rate Schedule

    
    def __init__(self, dates, time_schedule, unit_metric=TariffElemMetricUnit.EN_KWH, bill_period=TariffElemPeriod.YEARLY, unit_cost=TariffElemCostUnit.DOLLAR, name=None):
        """
        Constructor
        :param dates: see FixedTariff init
        :param time_schedule: TODO
        ...
        super(NEMCreditTariff, self).__init__(dates, time_schedule, unit_metric, unit_cost, name)
    ...
    def compute_yearly_bill(self,df,data_col=None):
        ...
        #print(df)
        # TODO:  change everything to arrays instead of pandas dataframes
        # Serena TODO: add NBC for positive net energy in the interval (hour/minute)
        ...
        for idx, df_day in df.groupby(df.index.date):
            ...
            daily_rate = self.rate_schedule.get_daily_rate(df_day.index[0])
            df_prices = self.get_daily_price_dataframe(daily_rate, df_day) #these are just one 
            avg_daily = df_prices['price'].sum()/len(df_prices['price'])
            
            #print('dfprices', df_prices)
            nsc_daily.append(avg_daily)
            ...
            # Cumulate the energy over the hour
                ...
                # if isinstance(daily_rate[0],float): #only tou
                #     nsc_flat = df_prices.loc[i, 'price'] 
                #     #print(nsc_flat)
                cost += mult_cost_unit * df_values_in_hour * df_prices.loc[i, 'price'] / mult_energy_unit
                # else:
                #     nsc_flat = 0.03 #DEFALUT
                #     #print('no nsc rate available')
        #nsc_flat = sum(nsc_daily)/len(nsc_daily)
        #print('any excess generation over the year will be compensated at:', nsc_flat, '$/kWh')
        if energy < 0: #if it's the entire year and more energy (negatives)
            #compensate
            #cost= mult_cost_unit * energy * nsc_flat 
          return energy, cost
    ...
    def compute_monthly_bill(self, df, data_col=None):
        """
        Compute the bill due to a net surplus compensation from net energy metering
        :param df: a pandas dataframe
        :return: a tuple (float, float) -> (cost, tot_energy)
        """
