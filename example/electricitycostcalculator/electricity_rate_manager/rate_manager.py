...
class ElectricityRateManager(object):
    ...
    DEFAULT_TARIFF_MAP = {str(TariffType.FIX_CUSTOM_CHARGE.value): ChargeType.FIXED,
                          ...
                          str(TariffType.NEM_ENERGY_CHARGE.value): ChargeType.NEM,
                          str(TariffType.NEM_ENERGY_CREDIT.value): ChargeType.NEM
                          }
#
    ...
    def compute_bill(self, df, column_data=None, monthly_detailed=False):
        ...
        while t_i <= df.index[-1]:
            ...
            for k in list(self.__tariffstructures.keys()):
                ...
                elif self.type_tariffs_map[k] == ChargeType.NEM:
                    ret[t_i.strftime("%Y")][k] = (0, 0) #why is the year (ie 2020) a key error?
                    ret[t_i.strftime("%Y-%m")][k] = (0, 0)
                else:
                    ret[t_i.strftime("%Y-%m")][k] = (0, 0)  # a tuple
                    ret[t_i.strftime("%Y")][k] = (0, 0) 
        ...
        for label, tariff_data in list(self.__tariffstructures.items()):
            ...
            for tariff_block in l_blocks:
                ...
                for time_label, bill_data in list(tariff_cost_list.items()):
                    #print('time_label:\n', time_label,'label:\n', label,'bill_ddata:\n', bill_data)
                    bill_dict.update({str(time_label) : {str(label) : bill_data}})
                    #if len(time_label)>4:
                    #print(time_label, label)
                    if (len(time_label)>4): #if it's not a year
                        #print(label, time_label, len(time_label))
                        self.update_bill_structure(ret[time_label], label, bill_data)
                    elif label == 'NEM_NSC': #add any other yearly bill types
                        #print(label, time_label, len(time_label))
                        self.update_bill_structure(ret[time_label], label, bill_data)
        ...
        if monthly_detailed is False:  # Aggregate all the months
            ...
            return self.aggregate_monthly_bill(ret),bill_dict
            #return ret
        else:
            return ret
    ...
    def print_aggregated_bill(self, bill_struct, verbose=True):
        ...
        if monthly_detailed is True:  # This supposes the bill is calculated per natural month of the calendar
            ...
            acc_tot = 0.0
            energy_tot = 0.0
            acc_per_chargetype = {ChargeType.FIXED: 0.0, ChargeType.ENERGY: 0.0, ChargeType.DEMAND: 0.0, ChargeType.NEM: 0.0}
            acc_per_label = {}
            ...
            for m_key, bill_per_label in list(bill_struct.items()):
                for lab_tariff, data in list(bill_per_label.items()):
                    ...
                    if self.type_tariffs_map[lab_tariff] is ChargeType.ENERGY:
                        energy_per_tariff = data[0]
                        energy_tot+=energy_per_tariff
                        #print('kWh energy??', energy_per_tariff)
        ...
        if verbose:
            ...
            print(("\n| Aggregated energy: {0} (kWh)".format(energy_tot)))
        ...
        return acc_tot, acc_per_chargetype, acc_per_label, energy_tot
